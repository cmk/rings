...
--
(.*) :: Presemiring a => Functor f => Functor g => f (g a) -> a -> f (g a)
f .* a = fmap (fmap (* a)) f
{-# INLINE (.*) #-}
...
-- V2 (V2 5 10) (V2 15 20)
--
(*.) :: Presemiring a => Functor f => Functor g => a -> f (g a) -> f (g a)
(*.) a = fmap (fmap (a *))
{-# INLINE (*.) #-}
-}
...
scale f = flip imapRep f $ \i x -> flip imapRep f (\j _ -> if i == j then x else zero)
{-# INLINE scale #-}
