...
-- | Use a linear transformation to map over the dual space.
--
mapTran :: Tran a b c -> Cov a b -> Cov a c
mapTran f g = Cov $ runCov g . runTran f
...
-- | Obtain a vector from a function on basis elements.
--
join :: Algebra a b => (b -> b -> a) -> Vec a b 
join = Op . joined
...
--
-- >>> cov (V2 3 4) !* V2 1 2 :: Int
-- 11
--
cov :: FreeCounital a f => f a -> Cov a (Base f)
cov f = Cov $ \k -> f `inner` tabulate k

-- | Obtain a covector from two basis elements.
--
cojoin :: Coalgebra a c => c -> c -> Cov a c
cojoin x y = Cov $ \k -> cojoined k x y 
