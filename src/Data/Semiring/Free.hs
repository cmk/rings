...
-- See < https://en.wikipedia.org/wiki/Covariance_and_contravariance_of_vectors#Definition >.
--
newtype Cov a c = Cov { runCov :: Vec a c -> a }
...
-- 11
--
cov :: FreeCounital f a => f a -> Cov a (Rep f)
...
--
cmap :: Mat a b c -> Cov a b -> Cov a c
cmap f g = Cov $ runCov g . vmap f
...
instance Presemiring a => Semigroup (Cov a b) where
  (<>) (Cov m) (Cov n) = Cov $ m + n

instance Semiring a => Monoid (Cov a b) where
  mempty = Cov zero

instance Ring a => Magma (Cov a b) where
  (<<) (Cov m) (Cov n) = Cov $ m - n

instance Ring a => Quasigroup (Cov a b)
instance Ring a => Loop (Cov a b)
instance Ring a => Group (Cov a b)
...
instance Sieve (Mat a) (Cov a) where
  sieve l b = Cov $ \k -> (l !# k) b 

instance PR.Representable (Mat a) where
  type Rep (Mat a) = Cov a
  tabulate f = Mat $ \k b -> runCov (f b) k
